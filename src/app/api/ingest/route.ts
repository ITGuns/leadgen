...
export const GET = withAuth(async () => {
  ...
  return Response.json({
    releases: await db.select().from(releases).orderBy(desc(releases.id)).limit(20),
    recentJobs: await db.select().from(jobs).orderBy(desc(jobs.id)).limit(25),
  });
});
...
export const POST = withAuth(async (req, identity) => {
  ...
  await audit(identity.email, "ingest.run", { states, jobId: job.id });

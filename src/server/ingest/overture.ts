...
export async function runOvertureExtract(ctx: JobContext): Promise<void> {
  ...
  const release = ensureReleaseRow("overture", releaseId, states);
  ...
  await withDuck(async (conn) => {
    for (let i = progress.stateIndex ?? 0; i < states.length; i++) {
      ...
      // Staging semantics: latest-wins per GERS id (delete+insert, chunked under the
      // SQLite bind limit). A gate-failed release never reaches conflation, so the
      // conflated `businesses` view stays on the previous release either way.
      db.transaction(() => {
        for (let c = 0; c < values.length; c += 200) {
          const chunk = values.slice(c, c + 200);
          db.delete(placesOverture)
            .where(sql`${placesOverture.gersId} IN (${sql.join(chunk.map((v) => sql`${v.gersId}`), sql`, `)})`)
            .run();
          db.insert(placesOverture).values(chunk).run();
        }
      });
      rowCounts[state] = values.length;
      ctx.checkpoint({ stateIndex: i + 1, rowCounts, at: ts });
    }
  });

  db.update(releases).set({ rowCounts }).where(eq(releases.id, release.id)).run();
  const gate = runOvertureGate(releaseId, states, rowCounts);
  if (!gate.pass) {
    failRelease(release.id, gate);
    throw new Error(`ingest gate FAILED for overture ${releaseId}: ${gate.failures.join("; ")} — previous release kept active`);
  }
  activateRelease("overture", release.id, gate);
  ctx.checkpoint({ gate });

  if (payload.chain) {
    const { enqueueJob } = await import("../jobs/worker");
    enqueueJob("ingest_fsq", { states, chain: true }, { dedupe: true });
  }
}

/** Row count sanity for the definition-of-done spot check. */
export function overtureRowCount(releaseId: string): number {
  const row = getDb()

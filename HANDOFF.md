...
## Monthly
1. Check the Overture releases page; bump `OVERTURE_RELEASE` in `.env` (and the taxonomy file if the release notes mention taxonomy changes).
...
- [ ] #K: PageSpeed + Google OAuth keys entered in Settings; Drive consent completed; folder ID set; monthly ceiling consciously chosen (default $0); optional Anthropic/Outscraper keys + console limits

...
## Session 5 · 2026-09-07 — Vercel + Supabase migration (D19–D21)
User-directed: deploy target is Vercel with the DB on Supabase. Full port done and green locally: schema → Drizzle **pg-core** (one Postgres schema, two drivers — postgres-js against the Supabase transaction pooler, or **PGlite** for keyless dev/CI/Docker; better-sqlite3 removed); the entire server went async; jsonb replaced json1 SQL everywhere. Serverless jobs: Vercel minute-cron → `/api/jobs/tick` (Bearer `CRON_SECRET`) runs time-boxed worker slices; deadline stops requeue without burning attempts; `after()` kicks on enqueue routes. Exports upload to the private Supabase Storage bucket `exports` with signed-URL downloads; secret store moved into the `secure_config` table (D8 revised); backup job self-skips on the shared DB (Supabase-managed, D21); `AUTH_TRUST_PLATFORM` switch for Deployment Protection (D20). DuckDB extract is workstation-only → writes straight into Supabase (`scripts/workstation-extract.ts`). Caught in verification: Playwright boots the web server BEFORE globalSetup, and deleting a live PGlite dir corrupts it (SQLite survived via POSIX unlink) — state reset moved into the webServer command. **218 tests + E2E 5/5 + lint + `next build` green. NOT deployed — waiting for the operator's explicit go (BLOCKERS B13–B15, HANDOFF → Deploy).**
